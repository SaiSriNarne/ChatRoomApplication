...
def clientthread(conn, addr):  # Common client function for each socket connection
    try:
        type_msg = conn.recv(
            RECV_BUFFER)  # initital msg type to distinguish between non registration and registration messages
            ...
            if first_time == True:
                ...
                if name_enter:
                    ...
                    if authenticate(name, pwd):  # If authenticated,i.e, present in local db, then do this
                        # store this in local database.txt
                        ...
                        for socket in SOCK_LIST:
                            if socket != conn:
                                active_list = active_list + SOCK_LIST[
                                    socket] + " "  # Adding to the active list names of all active users (represented by SOCK_LIST[socket])
                        try:
                        ...
                        conn.send(
                            'authentication_error'.encode())  # If authentication fails, i.e user-password not found, then
                        ...
                        try:
                            if data[0] != '@' and data[0] != '^':  # If this msg does not start with @ , then it is broadcast
                                ...
                                multicast_data(conn, list_of_socks, "\r" + SOCK_LIST[conn] + ':' + ':'.join(temp[1:]),
                                    ...
                                    list_of_names = list(map(lambda x: x.rstrip(), temp2))
                                    list_of_socks = list(map(lambda x: USERS_LIST[x], list_of_names))
                                    emoji(conn, list_of_socks, "\r" + SOCK_LIST[conn] + ':' + ':'.join(temp[1:]),True)
                            ...
                            broadcast_data(conn, "  %s went offline" % SOCK_LIST[
                                conn])  # To inform all about the user going offline
...
# authenticating from the MYSQLite
def authenticate(uname, pwd):
    is_valid = False
    dbconn = sqlite3.connect("chatroom.db")
    cursor = dbconn.cursor()
    cursor.execute("SELECT * FROM `clientdata` WHERE `username` = ? AND `password` = ?",
                   (uname, pwd))
...
# Adding a new user to the chatroom.db
def reg_user(uname, pwd):
    # we will now open /update file to maintain a database.txt
    dbconn = sqlite3.connect("chatroom.db")
    cursor = dbconn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS `clientdata` (mem_id INTEGER NOT NULL PRIMARY KEY  AUTOINCREMENT, username TEXT, password TEXT)")
    cursor.execute("INSERT INTO clientdata (username, password) VALUES (?,?)", (uname, pwd))
    dbconn.commit()
...
# To exchange emoji's
def emoji(sock, list_of_sock, message, emoji):
    for socket in list_of_sock:
        # Do not send the message to master socket and the client who has send us the message
        if socket in SOCK_LIST:
            try:
...
if __name__ == "__main__":
    ...
    host = '127.0.0.1'
    port = 1025  # Use port > 1024, below it all are reserved
    ...
    while 1:
        ...
        start_new_thread(clientthread, (conn,
                                        addr))  # start new thread takes 1st argument as a function name to be run, second is the tuple of arguments to the function.
